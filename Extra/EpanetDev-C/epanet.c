...
#ifdef DEBUG
void printFloats(float* f, int n) {
	for (int i = 0; i < n; i++) {
		printf("%f \n", f[i]);
	}
}

#define DEBUGS(valor) printf("En la funcion %s - %s - { %s }\n", __FUNCTION__, # valor, valor)
#define DEBUGI(valor) printf("En la funcion %s - %s - { %d }\n", __FUNCTION__, # valor, valor)
#define DEBUGF(valor) printf("En la funcion %s - %s - { %f }\n", __FUNCTION__, # valor, valor)
#define DEBUGL(valor) printf("En la funcion %s - %s - { %ld }\n", __FUNCTION__, # valor, valor)
#define DEBUG_ARRAYF(valores, n) printf("En la funcion %s - %s: \n", __FUNCTION__, #valores ); printFloats(valores, n)
...
*/
{
	DEBUGI(cindex); DEBUGI(ctype); DEBUGI(lindex); DEBUGF(setting); DEBUGI(nindex); DEBUGF(level);
   char   status = ACTIVE;
   long   t = 0;
...
*/
{
	DEBUGI(index); DEBUGI(n); DEBUG_ARRAYF(f, n); 
...
*/
{
	DEBUGI(index); DEBUGI(period); DEBUGF(value);
   if (!Openflag) return(102);
   if (index  <= 0 || index  > Npats) return(205);
...
*/
{
	DEBUGI(qualcode);DEBUGS(chemname);DEBUGS(chemunits); DEBUGS(tracenode);
...
void testPrint(char* arg, char* value) {
	printf("%s contiene el valor %s", arg, value);
}